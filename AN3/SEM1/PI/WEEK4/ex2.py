...
net = nn.Sequential(
    nn.Conv2d(3, 6, kernel_size=5, padding=2), nn.Sigmoid(),#aici am modificat 3 pentru avem trei canale adica R G B
    nn.AvgPool2d(kernel_size=2, stride=2),
    nn.Conv2d(6, 16, kernel_size=5), nn.Sigmoid(),
    nn.AvgPool2d(kernel_size=2, stride=2),
    nn.Flatten(),
    nn.Linear(16 * 6 * 6, 120), nn.Sigmoid(),#aici dimensiunea rezultată după convoluții și pooling, adică dimensiunea spațială a fiecărui feature map.
    nn.Linear(120, 84), nn.Sigmoid(),
    nn.Linear(84, 10))

X = torch.rand(1, 3, 32, 32)#aici
for layer in net:
    X = layer(X)
...
def load_data_fashion_mnist(batch_size, resize=None):
    ...
    mnist_train = torchvision.datasets.SVHN(
        root="../data", split = 'train', transform=trans, download=True)#aici
    mnist_test = torchvision.datasets.SVHN(
        root="../data", split = 'test', transform=trans, download=True)
    mnist_train, mnist_val = torch.utils.data.random_split(mnist_train, [43257, 30000],#aici
